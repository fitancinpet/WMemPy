...
print(f'The pointer to local player is located in {client.get_name()} at address {hex(local_player_offset)}')
# We have address of pointer to local player, reading it lands us with pointer to local player aka
# address, where local_player really is at in csgo memory, this changes each time you reload map / die etc...
local_player_pointer = client.read_dtype(local_player_offset, wintypes.DWORD())
# If you reverse engineer the player structure of CSGO, we can find some cool values, such as team, health etc...
...
print('Reversing the team number will make the game think we are on the enemy team and it will allow us to see enemies as if they were allies :)')
